...
//      if (::GetClipboardFormatNameA(pbase->m_uiMessage, szBuf, _countof(szBuf)))
  //       lpszMsgName = szBuf;
   ...
   {
#ifdef _WIN64
      //TRACE(::ca::trace::category_WinMsg, 4, "%s: hwnd=%p, msg = %s (%p, %p)\n",
        // lpszPrefix, pbase->m_hwnd, lpszMsgName,
         //pbase->m_wparam, pbase->m_lparam);
#else
    //  ::OutputDebugString(::ca::trace::category_WinMsg, 4, "%s: hwnd=0x%08X, msg = %s (0x%08X, 0x%08X)\n",
//         lpszPrefix, pbase->m_hwnd, lpszMsgName,
  //       pbase->m_wparam, pbase->m_lparam);
#endif
   }
   ...
   {
#ifdef _WIN64
//      ::OutputDebugString(::ca::trace::category_WinMsg, 4, "%s: hwnd=%p, msg = 0x%04X (%p, %p)\n",
  //       lpszPrefix, pbase->m_hwnd, lpszMsgName,
    //     pbase->m_wparam, pbase->m_lparam);
#else
//      ::OutputDebugString(::ca::trace::category_WinMsg, 4, "%s: hwnd=0x%08X, msg = 0x%04X (0x%08X, 0x%08X)\n",
  //       lpszPrefix, pbase->m_hwnd, lpszMsgName,
    //     pbase->m_wparam, pbase->m_lparam);
#endif
   }
...
//      if (::GetClipboardFormatNameA(lpmsg->message, szBuf, _countof(szBuf)))
  //       lpszMsgName = szBuf;
   ...
   {
#ifdef WIN64
//      TRACE(::ca::trace::category_WinMsg, 4, "%s: hwnd=%p, msg = %hs (%p, %p)\n",
  //       lpszPrefix, pMsg->hwnd, lpszMsgName,
    //     pMsg->wParam, pMsg->lParam);
#else
//      ::OutputDebugString(::ca::trace::category_WinMsg, 4, "%s: hwnd=0x%08X, msg = %hs (0x%08X, 0x%08X)\n",
