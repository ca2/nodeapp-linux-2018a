...
namespace win
{
   ...
   CLASS_DECL_VMSLNX HINSTANCE   LoadLibrary(const char * lpsz);
   CLASS_DECL_VMSLNX WINBOOL        SHGetSpecialFolderPath(HWND hwnd, string &str, int csidl, WINBOOL fCreate);
   CLASS_DECL_VMSLNX DWORD       GetFileAttributes(const char * lpFileName);
   CLASS_DECL_VMSLNX WINBOOL        CreateDirectory(const char * lpPathName, LPSECURITY_ATTRIBUTES lpSecurityAttributes);
   CLASS_DECL_VMSLNX DWORD       GetCurrentDirectory(string & str);
   CLASS_DECL_VMSLNX DWORD       GetTempPath(string & str);
//   CLASS_DECL_VMSLNX LONG        RegQueryValue(HKEY hkey, const char * lpszSubKey, string & str);
   CLASS_DECL_VMSLNX HICON       ExtractIcon(HINSTANCE hInst, const char * lpszExeFileName, UINT nIconIndex);
   CLASS_DECL_VMSLNX WINBOOL        DeleteFile(const char * lpFileName);
   CLASS_DECL_VMSLNX int         GetMenuStringW(HMENU hMenu, UINT uIDItem, string & str, UINT flags);
...
struct CLASS_DECL_VMSLNX __EXCEPTION_LINK
{
...
struct CLASS_DECL_VMSLNX __EXCEPTION_CONTEXT
{
...
_PNH CLASS_DECL_VMSLNX AfxGetNewHandler();
...
//CLASS_DECL_VMSLNX WINBOOL AfxRegisterClass(WNDCLASS* lpWndClass);

// helper to initialize rich edit 1.0 control
CLASS_DECL_VMSLNX WINBOOL AfxInitRichEdit();
// helper to initialize rich edit 2.0 control
CLASS_DECL_VMSLNX WINBOOL AfxInitRichEdit2();

CLASS_DECL_VMSLNX LRESULT CALLBACK AfxWndProc(HWND hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam);
...
//CLASS_DECL_VMSLNX LONG AfxDelRegTreeHelper(HKEY hParentKey, const string & strKeyName);


// Advanced initialization: for overriding default diagnostics
CLASS_DECL_VMSLNX WINBOOL AfxDiagnosticInit(void);


CLASS_DECL_VMSLNX ::win::thread * AfxGetThread();
CLASS_DECL_VMSLNX void AfxSetThread(::radix::thread * pthread);
CLASS_DECL_VMSLNX MSG* AfxGetCurrentMessage();

CLASS_DECL_VMSLNX void AfxEndThread(::radix::application * papp, UINT nExitCode, WINBOOL bDelete = TRUE);

CLASS_DECL_VMSLNX void AfxInitThread();
CLASS_DECL_VMSLNX void AfxTermThread(::radix::application * papp, HINSTANCE hInstTerm = NULL);
...
//CLASS_DECL_VMSLNX WINBOOL AfxWinInit(__in HINSTANCE hInstance, __in HINSTANCE hPrevInstance,
  // __in_z LPTSTR lpCmdLine, __in int nCmdShow);
CLASS_DECL_VMSLNX void AfxWinTerm();

CLASS_DECL_VMSLNX ::radix::application* AfxGetApp();
CLASS_DECL_VMSLNX ::user::interaction* AfxGetMainWnd();
//CLASS_DECL_VMSLNX HINSTANCE CLASS_DECL_VMSLNX System.m_hInstance;
...
CLASS_DECL_VMSLNX HINSTANCE AfxGetResourceHandle();
CLASS_DECL_VMSLNX HINSTANCE AfxFindStringResourceHandle(UINT nID);

CLASS_DECL_VMSLNX void AfxDeleteObject(HGDIOBJ* pObject);
