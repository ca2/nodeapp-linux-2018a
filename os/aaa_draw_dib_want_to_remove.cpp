...
namespace win
{
   ...
   WINBOOL draw_dib::open()
   {
      return (m_hdrawdib = DrawDibOpen()) != NULL;
   }

   WINBOOL draw_dib::close()
   {
      if(m_hdrawdib == NULL)
         return TRUE;
      WINBOOL b = DrawDibClose(m_hdrawdib);
      m_hdrawdib = NULL;
      return b;
   }

   WINBOOL draw_dib::Begin (::ca::graphics * pdc, int dxDest, int dyDest,
   ...
   WINBOOL draw_dib::End ()
   {
      return DrawDibEnd ( m_hdrawdib );
   }

   WINBOOL draw_dib::draw (::ca::graphics * pdc, int xDst, int yDst,
   ...
   WINBOOL draw_dib::draw (::ca::dib *dib, ::ca::graphics * pdc, int xDst, int yDst,
   ...
   WINBOOL draw_dib::ProfileDisplay ( LPBITMAPINFOHEADER lpbi )
   {
   ...
   WINBOOL draw_dib::ChangePalette ( ::ca::draw_dib * pdd, int iStart, int iLen, LPPALETTEENTRY lppe )
   {
      UNREFERENCED_PARAMETER(pdd);
      return DrawDibChangePalette ( m_hdrawdib, iStart, iLen, lppe );
   }

   UINT draw_dib::Realize ( ::ca::graphics * pdc, WINBOOL fBackground )
   {
      return DrawDibRealize ( m_hdrawdib,  (HDC) pdc->get_os_data(),  fBackground );
   }

   WINBOOL draw_dib::Start ( LONG rate )
   {
      return DrawDibStart ( m_hdrawdib, rate );
   }

   WINBOOL draw_dib::Stop ()
   {
      return DrawDibStop ( m_hdrawdib );
   }

   WINBOOL draw_dib::time ( LPDRAWDIBTIME lpddtime )
   {
